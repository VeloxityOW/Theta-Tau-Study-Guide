...
export default function RootLayout({ children }) {
  return <html lang="en"><head><style>{`
    ...
    #initial-launch-mask.exiting{pointer-events:none;animation:initial-ripple-out 700ms cubic-bezier(.22,.8,.25,1) both;-webkit-mask-image:radial-gradient(circle at 50% 50%,transparent 0%,transparent var(--reveal-radius),#000 calc(var(--reveal-radius) + .3%),#000 100%);mask-image:radial-gradient(circle at 50% 50%,transparent 0%,transparent var(--reveal-radius),#000 calc(var(--reveal-radius) + .3%),#000 100%)}
