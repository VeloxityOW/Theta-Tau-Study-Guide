...
(function () {
  ...
  function handleAction(action, event) {
    ...
    if (name === "show-members") setView("members");
    if (name === "open-invites") window.location.assign("/invites");
    ...
    if (name === "toggle-quiz") toggleQuizVisibility(action.dataset.quiz);
  ...
  function getDeck(reviewOnly = false) {
    ...
    return questions.filter(q => {
      ...
      if (!window.THETA_PORTAL?.isStaff && !window.THETA_PORTAL?.visibleQuizzes?.has(q.quiz)) return false;
  ...
  function questionEditorHtml(question) {
    return `<p class="eyebrow">${state.creatingQuestion ? "Add question" : "Edit question"}</p><label>Question<input id="staffQuestionText" class="answer-input" value="${escapeAttribute(question.q)}" /></label><label>Answer<textarea id="staffQuestionAnswer" class="answer-input">${escapeHtml(formatAnswer(question.a))}</textarea></label><label>Quiz<input id="staffQuestionQuiz" class="answer-input" value="${escapeAttribute(question.quiz)}" /></label><div class="hero-actions"><button class="primary-btn" data-action="save-question-edit" data-id="${escapeAttribute(question.id)}" type="button">Save changes</button>${question.isCustom ? `<button class="ghost-btn danger" data-action="remove-question" data-id="${escapeAttribute(question.id)}" type="button">Remove</button>` : ""}<button class="ghost-btn" data-action="cancel-question-edit" type="button">Cancel</button></div>`;
  }
  ...
  async function toggleQuizVisibility(quiz) {
    const portal = window.THETA_PORTAL;
    if (!portal?.supabase || !quiz) return;
    const isVisible = !portal.quizReleases?.[quiz];
    const { error } = await portal.supabase.from("quiz_releases").upsert({ quiz_name: quiz, is_visible: isVisible });
    if (error) return toast("Run the latest quiz-release migration, then try again.", "bad");
    portal.quizReleases[quiz] = isVisible;
    portal.visibleQuizzes = new Set(Object.entries(portal.quizReleases).filter(([, visible]) => visible).map(([name]) => name));
    toast(`${quiz} is now ${isVisible ? "visible" : "hidden"} for PNMs.`, "ok");
    renderClassSettings();
  }
